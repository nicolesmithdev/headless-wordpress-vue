...
define( 'CHILD_THEME_NAME', 'Toning With Twins' );
define( 'CHILD_THEME_URL', 'https://www.toningwithtwins.com/' );
...
function genesis_sample_localization_setup() {
	...
	load_child_theme_textdomain( 'toningwithtwins', get_stylesheet_directory() . '/languages' );
...
function genesis_sample_enqueue_scripts_styles() {
	...
	wp_enqueue_style(
		...
		'//fonts.googleapis.com/css?family=Lato:400,400i,700,700i|Playfair+Display:400,400i,700,700i|Raleway:400,400i,500,500i,600,600i,700,700i,800,800i',
		array(),
		CHILD_THEME_VERSION
	);
...
// Adds support for above content widget 
...
function custom_logo() {
	echo '<style type="text/css">.login h1 a { background-image:url("' . get_stylesheet_directory_uri() . '/images/logo.png") !important; background-size:auto; height:84px; margin:0 auto; width:320px; }</style>';
}
add_action('login_head', 'custom_logo');

// Change logo link on login page
function loginpage_custom_link() { return esc_url(home_url('/')); }
...
// Adds toolbar to top of header
remove_action('genesis_header', 'genesis_header_markup_open', 5);
add_action('genesis_header', 'header_markup_open', 5);
function header_markup_open() {
...
add_action('genesis_header', 'toolbar', 7);
function toolbar() {
	echo '<div id="toolbar"><div class="wrap">';
		wp_nav_menu( array(
			'container'			=> 'nav',
		...
		echo '<div>';
			wp_nav_menu( array(
				'container'			=> 'nav',
				'container_class'	=> 'genesis-nav-menu genesis-responsive-nav nav-mobile',
...
function fc_opengraph() {
	if( is_single() || is_page() ) {
		...
		echo '<meta name="twitter:site" content="@toningwithtwins" />';
		echo '<meta name="twitter:creator" content="@toningwithtwins" />';
	}
}
...
function sp_custom_footer() {
	echo '<div class="site-info"><div class="wrap">';
		echo '<div>&copy; 2017 - ' . date("Y") . ' Nicole Smith, ' . get_bloginfo('name') . '. All Rights Reserved. <a href="' . esc_url(home_url('/privacy/')) . '">Privacy Policy</a>.</div>';
	echo '</div></div>';
}
...
// Relocate post info above entry title
...
// Add 'Read More' button below post excerpts/content on archive
function custom_add_read_more() {
	if ( is_singular() || is_post_type_archive('tribe_events') ) {
		return;
	}

	printf( '<p><a href="%s" class="more-link button">%s</a></p>', get_permalink(), esc_html__( 'Continue Reading' ) );
}
add_action('genesis_entry_content', 'custom_add_read_more');

// Add featured image on single post
add_action('genesis_entry_header', 'themeprefix_featured_image', 10);
